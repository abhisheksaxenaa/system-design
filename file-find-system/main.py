...
def main():
    ...
    extension_filter = ExtensionFilter(extension="txt")
    result = FileSearchEngine.search(files, extension_filter)
    print(f"Files with .txt extension: {len(result)}")
    for file in result:
        print(file)

    # Testing the SizeFilter
    size_filter = SizeFilter(min_size=10 * 1024, max_size=1 * 1024 * 1024)
    result = FileSearchEngine.search(files, size_filter)
    print(f"\nFiles with size between 10KB and 1MB: {len(result)}")
    for file in result:
        print(file)
